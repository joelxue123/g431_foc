  ...
  uint32_t blockSize)
{
  ...
  while (blkCnt > 0U)
  {
    ...
    inVLL = vmovl_int16_t(vget_low_int16_t(inVLO));
    inVLH = vmovl_int16_t(vget_high_int16_t(inVLO));
    inVHL = vmovl_int16_t(vget_low_int16_t(inVHI));
    inVHH = vmovl_int16_t(vget_high_int16_t(inVHI));
